...
namespace gb_o_tron
{
    class WAVOutput : IDisposable
    {
        ...
        public void AddSamples(byte[] buffer, int samples)
        {
            if (recording)
            {
        ...
        public void CompleteRecording()
        {
            if (recording)
            {
                ...
                int subchunk2Size = samples * 2 * (8 / 8);
                ...
                outputWriter.Write((short)2); //Channels
                ...
                outputWriter.Write((short)8); //Bits per Sample
